classdef SampleSelection < eegtoolkit.preprocessing.PreprocessingBase
    ...
    methods
        ...
        function out = process(CS,in)
            ...
            elseif size(in,2) == 1
                CS.samplingRate = in.samplingRate;
                if(length(CS.channels) > 0)
                    if(length(CS.sampleRange) > 2)
                        signal = in.signal(CS.channels,CS.sampleRange);
                        out = eegtoolkit.util.Trial(signal ...
                            ,in.label,in.samplingRate,in.subjectid,in.sessionid);
                    elseif(length(CS.sampleRange) > 0)
                        %channels AND sampleRange
                        signal = in.signal(CS.channels,CS.sampleRange(1):CS.sampleRange(2));
                        out = eegtoolkit.util.Trial(signal ...
                            ,in.label,in.samplingRate,in.subjectid,in.sessionid);
                    else
                        %ONLY channels
                        signal = in.signal(CS.channels,:);
                        out = eegtoolkit.util.Trial(signal ...
                            ,in.label,in.samplingRate,in.subjectid,in.sessionid);
                    end
                else
                    if(length(CS.sampleRange) > 2)
                        signal = in.signal(:,CS.sampleRange);
                        out = eegtoolkit.util.Trial(signal ...
                            ,in.label,in.samplingRate,in.subjectid,in.sessionid);
                    elseif(length(CS.sampleRange) > 0)
                        %ONLY sampleRange
                        signal = in.signal(:,CS.sampleRange(1):CS.sampleRange(2));
                        out = eegtoolkit.util.Trial(signal ...
                            ,in.label,in.samplingRate,in.subjectid,in.sessionid);
                    else
                        %NOTHING (?)
